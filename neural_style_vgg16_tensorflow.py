...
if __name__ == '__main__':
    ...
    x_content=tf.placeholder(tf.float32, [1, img_nrows, img_ncols, 3], 'Content_Input')
    x_style = tf.placeholder(tf.float32, [1, img_nrows, img_ncols, 3], 'Style_Input')
    x_generate=tf.Variable(tf.truncated_normal([1, img_nrows, img_ncols, 3]),name='Generated_image' )
    x = K.concatenate([x_content,x_style,x_generate], axis=0)

    _,y_block_conv1, y_block5_conv2 = NSVI.inference(x,layerName_list_vgg16,parameters_vgg16)
    ...
    content_img = np.array([skimgtr.resize(content_img, (img_nrows, img_ncols))])-mean
    #imageio.imwrite('content_resize.jpg',content_img[0])

    style_img = imageio.imread(style_img_file)
    style_img = np.array([skimgtr.resize(style_img, (img_nrows, img_ncols))])-mean
    ...
    for i in range(100):
        loss_trained,_ = sess.run([loss,train_step],{x_content:content_img,x_style:style_img})
